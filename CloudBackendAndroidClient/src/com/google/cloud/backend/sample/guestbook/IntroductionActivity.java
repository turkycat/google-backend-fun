...
public class IntroductionActivity extends Activity implements OnIntroNavListener
{
	...
	protected void onCreate( Bundle savedInstanceState )
	{
		...
		fragmentTransaction.add( R.id.intro_frame, new IntroFirstFragment(), IntroFirstFragment.TAG );
	...
	public void done( boolean skipFutureIntro )
	{
		...
		if( fragment != null )
		{
			fragmentTransaction.remove( getFragmentManager().findFragmentByTag( IntroThirdFragment.TAG ) );
		}
		fragmentTransaction.commit();
		finish();
	...
	{
		SharedPreferences settings = getSharedPreferences( GuestbookActivity.GUESTBOOK_SHARED_PREFS, Context.MODE_PRIVATE );
		if( settings != null )
		{
